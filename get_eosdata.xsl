...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common"
 xmlns:str="http://exslt.org/strings">
 	<xsl:key name="sets" match="set" use="@scale"/>
	<xsl:output method="xml" indent="yes"/>
 	...
 	<xsl:template name="iter">
 		...
 		<graph>
		<xsl:for-each select="//set[generate-id() = generate-id(key('sets',@scale)[$ind])]">
			<xsl:variable name="uniquescale"><xsl:value-of select="@scale"/></xsl:variable>
				
						<!-- Define path here -->
						
						<xsl:variable name="fullpath">
							<xsl:text><xsl:value-of select="/rootdir/@path"/></xsl:text>
	...
	<xsl:template match="/">
		<xsl:variable name="initsc" select="/experiment/set[last()]/@scale"></xsl:variable>
		<xsl:variable name="count" select="count(//set[@scale = $initsc])"/>
		<xsl:call-template name="iter">
